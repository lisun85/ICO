...
contract ICO {
    ...
    function release() external onlyAdmin()  tokensNotReleased() { // icoEnded() <= took this out as TIME funciton does not work
    ...
    function withdraw(address payable to, uint amount) external onlyAdmin() tokensReleased() { //icoEnded() <= took this out as TIME funciton does not work
        to.transfer(amount);    // DON'T UNDRESTAND HOW WITHDRAWAL AN ERC20 TOKEN CAN AUTOMATICALLY DEPOSIT TO THE RECEIPENT'S WALLET. HOW DOES THE RECIPIENT HAVE AN ALREADY READY WALLET? 
    }
    
    // function refill(address payable _to, uint amount) external payable {
    //     _to.transfer(amount);
    // }
